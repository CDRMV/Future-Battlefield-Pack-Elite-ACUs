...
-- Nomads ACU doesn't work in Steam Edition, because the Nomads are only available in FAF
    ...
    SetOrbitalBombardEnabled = function(self, condition)
        ...
        if condition then
            ...
            if not self:HasEnhancement( 'IntelProbe' ) and not self:HasEnhancement( 'IntelProbeAdv' ) then
                -- self.OrbitalUnit:SetMovementTarget(false) .. Causes an Error if the Nomads ACU is killed No Explosion
            end
        end
    end,
